        ...
        <Card className={` ${isCurrentMonth ? "" : "opacity-50"} overflow-y-auto`}>
            ...
            <CardContent onClick={handleClick} className="flex flex-1 flex-col items-start space-y-2 cursor-pointer max-sm:space-y-1 max-sm:px-1 ">
                {tasks.length > 0 && tasks.map((task) => (
                    <div key={task.id} className="w-full px-2 bg-accent rounded border overflow-hidden text-ellipsis whitespace-nowrap">
                        <p className={`text-sm max-sm:text-xs ${task.completed ? "line-through text-muted-foreground" : " text-foreground"}`}>{task.title}</p>
                    </div>
                ))}
            </CardContent>
        </Card>
